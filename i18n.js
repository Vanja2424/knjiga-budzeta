...
(function(root){
  const EN = {
    ...
    'Srednji kurs NBS{0}: {1}{2}': 'NBS middle rate{0}: {1}{2}',
    'Kursevi': 'Exchange rates', 'Preračunavanje': 'Converter', 'Izvor kursa': 'Rate source', 'Zameni smer': 'Swap direction', 'Iz valute': 'From currency', 'U valutu': 'To currency',
    'Traži valutu…': 'Search currency…', 'Traži valutu': 'Search currency', 'U unosu': 'In entry', 'Srednji kurs': 'Middle rate', 'Kupovni': 'Buying', 'Prodajni': 'Selling',
    'Zvezdicom biraš koje valute se nude pored iznosa pri unosu (prihodi, rashodi, ponavljajuće, prozor za unos).': 'Use the star to choose which currencies are offered next to the amount when adding entries (income, expenses, recurring, entry window).',
    'Srednji kurs NBS{0}': 'NBS middle rate{0}', 'Tržišni kurs (rezervni izvor){0}': 'Market rate (backup source){0}', ' — nema interneta, prikazan je poslednji preuzet kurs.': ' — offline, showing the last downloaded rates.',
    'Izvor NBS kursa (kurs.resenje.org) trenutno ne odgovara, pa se koristi open.er-api.com. Kurs je blizu NBS, ali nije zvaničan; čim NBS izvor proradi, aplikacija se sama vraća na njega.': 'The NBS rate source (kurs.resenje.org) isn’t responding, so open.er-api.com is used. The rate is close to NBS but not official; the app switches back as soon as NBS works again.',
    'Izvor: kurs.resenje.org (zvanična kursna lista NBS). Ako ne radi, koristi se rezervni izvor open.er-api.com.': 'Source: kurs.resenje.org (official NBS rates). If it’s down, the backup source open.er-api.com is used.',
    'Ukloni iz unosa': 'Remove from entry', 'Nudi pri unosu': 'Offer when adding', 'Nema valute za tu pretragu.': 'No currency matches that search.', 'kurs {0}': 'rate {0}', ' za {0}': ' for {0}', ' (nema interneta — prikazan poslednji preuzet kurs)': ' (offline — showing the last downloaded rates)',
